...
Chess has been a benchmark of the computing world for over a century, early chess engines such as uch as Alan Turing TuroChamp have relied on ppo traditional search algorthms. Recently the chess world has been dominated by deep learning based models such as DeepMind's AlphaZero. Instead of evaluating the strength of board states through human defined rule systems, they learn through reigorous self-play leading to superhuman performance.

\subsection{Human-Like Chess AI}

This superhuman level of play results in models that approach games in very different ways that human players. This in addition to models being aritificially weakened by reducing their search space and purposely choosing non optimal moves leads to confusing experiences when paired with human players. The Maia Project, by McIlroy-Young et al., represents a significant step in this direction. Maia is desigend to be aligned with human play, this is done by transfer learning on a large dataset of human games with the goal of predict the move a human player would make in a given position. This results in a series of models that can affectively mimic human play at various skill levels.

\subsection{Dataset and Training Data}

The Lichess database is a publicly available collection of human played games from the Lichess.org platform. Containing billions of games spanning a wide range of player skill levels from beginner to grandmaster. Alongside the chess moves lichess also provides metadata alongside each game containing important data such as player ELO ratings, game outcomes (win/loss), and time controls (the time limit of the game, e.g. Rated Classical).
...
Rylee further improves upon Maia's capabilities by including opening move predictions supporting a broader player base (700-2500 ELO vs. 1100-1900), and incorporating diverse game types. This allows Rylee to be a far more more practical tool as it covers a broader skill range and
...
\textbf{Larger Datasets:} Expanding beyond our current data set of fifteen thousand games would likely lead to significant performance improvements.

\textbf{Time-Conditioned Training:} Including remaining time and time control as input features would allow the model to capture how time affects the way that humans make decisions further improving it's ability to play under different time controls.

\textbf{Cross-Validation:} Implementing k-fold cross-validation would provide more robust performance estimates. At our current speeds it would take around 15 days to complete a cross validation of 5.

\subsubsection{Feature Additions}

\textbf{ELO Prediction Model:} Adding an additional RNN based model would allow for Rylee to dynamically adjust in real time to player skill and improve the learning process.

\textbf{Blunder Detection:} The addition of an auxillary head to determine wether or not a player is likely to respond with a blunder (an incorrect move) would allow for MAIA to identify areas where players at certain skill levels struggle. This would also allow for greater post game analysis to further augment the learning process.
