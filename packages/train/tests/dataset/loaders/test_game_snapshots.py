...
class TestGameSnapshotsDataset:
    ...
    @patch("packages.train.src.dataset.loaders.game_snapshots.count_processed_snapshots")
    def test_num_indexes_exceeds_database_count(self, mock_count_processed_snapshots):
        """Test ValueError raised when num_indexes exceeds database snapshot count."""
        mock_count_processed_snapshots.return_value = 100  # Mock snapshot count
    ...
    def test_fen_to_tensor(self):
        ...
        tensor = ProcessedSnapshotsProcessor.fen_to_tensor(fen)
    ...
    def test_encode_result(self):
        ...
        result_win = ProcessedSnapshotsProcessor.encode_result("1-0", "w")
        result_draw = ProcessedSnapshotsProcessor.encode_result("1/2-1/2", "b")
        result_loss = ProcessedSnapshotsProcessor.encode_result("0-1", "w")
    ...
    def test_encode_turn(self):
        ...
        turn_white = ProcessedSnapshotsProcessor.encode_turn("w")
        turn_black = ProcessedSnapshotsProcessor.encode_turn("b")
    ...
    def test_normalize_elo(self):
        ...
        result = ProcessedSnapshotsProcessor.normalize_elo(2000, 1500)
    ...
    @patch("packages.train.src.dataset.loaders.game_snapshots.count_processed_snapshots")
    def test_len_method(self, mock_count_processed_snapshots):
        """Test the __len__ method for dataset size."""
        mock_count_processed_snapshots.return_value = 100
    ...
    def test_encode_result_black_perspective(self):
        ...
        assert ProcessedSnapshotsProcessor.encode_result("0-1", "b").item() == 1.0
        # White wins (1-0) from black's perspective = loss (0.0)
        assert ProcessedSnapshotsProcessor.encode_result("1-0", "b").item() == 0.0
    ...
    def test_normalize_elo_typical_rating(self):
        ...
        result = ProcessedSnapshotsProcessor.normalize_elo(1600, 1600)
    ...
    def test_normalize_elo_extreme_ratings(self):
        ...
        result = ProcessedSnapshotsProcessor.normalize_elo(2800, 800)
